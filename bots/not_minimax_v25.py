...
def check_edge_cases(board):
    board_string = make_string_from_board(board)
    if board_string == 'OOXO..T......TOXXXO.TXOOXXOT......TX.....TX.....':
        return 5
...
def get_next_move(board, token):
    ...
    return_move = check_edge_cases(board)
    if return_move is not None:
        print('MOVE: Edge Case')
        return return_move

    sorted_open_opportunities = None
    open_opportunities_score = {}
    ...
    for move in preferred_locations:
        ...
        if score > 0:
            open_opportunities_score[move] = score

        if open_opportunities_score:
            sorted_open_opportunities = sorted(open_opportunities_score.items(), key=itemgetter(1))
            if sorted_open_opportunities[0][1] > 1:
                print('MOVE: making strong opportunities')
                return sorted_open_opportunities[0][0]

    sorted_enemy_opportunities = None
    enemy_opportunities_score = {}
    ...
    for move in available_moves:
        ...
        if score > 0:
            enemy_opportunities_score[move] = score

        if enemy_opportunities_score:
            sorted_enemy_opportunities = sorted(enemy_opportunities_score.items(), key=itemgetter(1))
            if sorted_enemy_opportunities[0][1] > 1:
                print('MOVE: blocking strong enemy opportunities')
                return sorted_enemy_opportunities[0][0]

    if sorted_open_opportunities:
        print('MOVE: making weak opportunities')
        return sorted_open_opportunities[0][0]

    if sorted_enemy_opportunities:
        print('MOVE: blocking weak enemy opportunities')
        return sorted_enemy_opportunities[0][0]
